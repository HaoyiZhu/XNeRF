...
class XNeRFSingleScene(torch.utils.data.Dataset):
    """
    Class for X-NeRF single scene dataset.
    ...
    def _get_inputs(
        self, coords: np.ndarray, feats: np.ndarray
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Get the discrete / voxelized inputs for ME models.

        Args:
            coords (``numpy.ndarray``): N x 3 numpy array of the original point cloud coordinates.
            feats (``numpy.ndarray``): N x M numpy array of the original point cloud features.

        Return:
            A tuple (discrete_coords, discrete_feats, keep_region)
            - ``torch.Tensor`` - discrete coordinates of input point cloud.
            - ``torch.Tensor`` - discrete features of input point cloud.
            - ``torch.Tensor`` - Indicates the pruning regions.
        """
    ...
    def _get_pcd(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Merge the multi-view point clouds.

        Return:
            A tuple (coords, feats)
            - ``numpy.ndarray`` - N x 3 coordinates.
            - ``numpy.ndarray`` - N x 3 features.
        """
    ...
    def _set_keep_region(self, discrete_coords: torch.Tensor) -> torch.Tensor:
        """
        Set the keep region which is useful for the pruning operations in ME models.
        """
    ...
    def _merge_pointclouds(
        ...
        """
        Merge two point clouds of different views.

        Args:
            pcd1 (``open3d.geometry.PointCloud``): The first point cloud.
            pcd1 (``open3d.geometry.PointCloud``): The second point cloud.
        ...
        merged_pcd = self._construct_pcd(merged_points, merged_colors)
    ...
    def _rgbd_to_pointcloud(
        ...
        Args:
            ...
            depth_scale (``int``): Scale ratio converting depth image from millimeters to meters. Default: ``1000``.
            use_o3d (``bool``): Whether to use open3d's built-in method. Default: ``False``

        Return:
            A tuple (pcd, color, depth)
        ...
        if use_o3d:
            ...
            pcd = self._construct_pcd(points, colors / 255)

        return pcd, color, points_z * mask

    def _filter_pointclouds(
        self,
        ...
        """
        Filter a group of multi-view point clouds.

        Args:
            pcds (``List[o3d.geometry.PointCloud]``): A list of open3d point clouds.
            num_neighbors (``int``): ``nb_points`` parameter for ``o3d.geometry.PointCloud.remove_radius_outlier``
                    and ``o3d.geometry.PointCloud.remove_statistical_outlier`` method.
            std_ratio (``float``): ``std_ratio`` parameter for ``o3d.geometry.PointCloud.remove_statistical_outlier`` method.
            radius (``float``): ``radius`` parameter for ``o3d.geometry.PointCloud.remove_radius_outlier`` method.

        Return:
            A list of filtered point clouds.
        """
    ...
    def _load_items(self) -> None:
        """
        Load training and testing data, including points clouds, rgbs, depths, etc. to memory.
        """
        ...
        for view_id in self.calib:
            ...
            pcd, rgb, depth = self._rgbd_to_pointcloud(
                rgb_path,
                depth_path,
                self.calib[view_id]["intrinsic"][0:2],
    ...
    def _extrinsic_to_camera_pose(self, extrinsic):
        """
        Converting an extrinsic matrix to a camera pose.

        Args:
            extrinsic (``numpy.ndarray``): The extrinsic matrix.
    ...
    def _rotation_matrix_axis(self, dim, theta):
        """
        Generate a rotation matrix along an axis.
        """
